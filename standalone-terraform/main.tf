...
output "public_endpoint" {
    value = rediscloud_subscription_database.autoscale-database.public_endpoint
}

output "private_endpoint" {
    value = rediscloud_subscription_database.autoscale-database.private_endpoint
}

output "autoscaler_vm_ip" {
    value = google_compute_instance.autoscaler-vm.network_interface[0].access_config[0].nat_ip
}

output "prometheus_vm_ip" {
    value = google_compute_instance.autoscale-vm-prometheus.network_interface[0].access_config[0].nat_ip
}

output "autoscaler_dns" {
    value = nonsensitive(google_dns_record_set.autoscaler_dns.name)
}

output "prometheus_dns" {
    value = nonsensitive(google_dns_record_set.autoscale_prometheus_dns.name)
}